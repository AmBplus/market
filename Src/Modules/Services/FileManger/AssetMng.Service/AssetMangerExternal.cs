...
{
    public class AssetMangerExternal : IAssetManger
    {
        public AssetMangerExternal(ILogger<AssetMangerExternal> logger
        ...
        private ILogger<AssetMangerExternal> Logger { get; }
        private IApplicationPathHelper FilePathHelper { get; }
        private ApplicationSettings Settings { get; }
        private HttpContext Context { get; }
        private HttpClient HttpClient { get; }
        public async Task<string> GetProfileBase64Async(string profileName)
        {
            ...
            // URL تصویر
            var imageUrl = $"{Settings.AvatarBaseRootPath}/{profileName}";

            try
            {
                // دانلود تصویر به صورت بایت آرایه
            ...
            {
                Logger.LogError($"Error fetching image from URL: {imageUrl} \n Exception: {ex.Message}");
                return string.Empty;
            }
        }
