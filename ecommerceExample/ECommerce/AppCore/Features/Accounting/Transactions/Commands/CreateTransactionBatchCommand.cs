...
namespace ECommerce.AppCore.Features.Accounting.Transactions.Commands;
...
public class CreateTransactionBatchHandler
{
    ...
    public CreateTransactionBatchHandler(ECommerceDbContext context) => _context = context;
    ...
    public async Task<ResultOperation<Guid>> Handle(CreateTransactionBatchCommand command, CancellationToken ct)
    {
        ...
        return ResultOperation<Guid>.ToSuccessResult(batchId, "دسته تراکنش با موفقیت ثبت شد");
    }
}
